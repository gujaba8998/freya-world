...
/* ---------------- streak / weekly loot box helpers ---------------- */
const dayKey = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
function weekStart(now = new Date()) {   // Monday 00:00 of the week containing `now`
  const d = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  d.setDate(d.getDate() - ((d.getDay() + 6) % 7));
  return d;
}
const weekKey = (d = new Date()) => dayKey(weekStart(d));
const STREAK_BONUS = { 3: 10, 5: 15, 7: 25, 14: 50, 30: 100 };  // consecutive days → bonus stars
const LOOT_GOAL = 5;   // approved missions per week to unlock the weekly surprise box

/* ---------------- Thai read-aloud (Web Speech API) ----------------
   Voice availability depends on the device: Android/Chrome ship a Thai voice,
   iOS needs the Thai Siri voice installed. Returns false when unsupported so
   callers can show a friendly toast instead of failing silently. */
function speakThai(text) {
  try {
    ...
    u.lang = 'th-TH';
    u.rate = 0.92;
    const v = synth.getVoices().find(v => v.lang && v.lang.toLowerCase().startsWith('th'));
    if (v) u.voice = v;
    synth.speak(u);
    return true;
...
function playSound(kind, enabled) {
  ...
  try {
    ...
    const seq = kind === 'reward' ? [523, 659, 784, 1047]
      : kind === 'tab' ? [660]
      : kind === 'star' ? [1319, 1568]                        // "ติ๊ง!" เมื่อได้ดาว
      : kind === 'levelup' ? [523, 659, 784, 1047, 1319]      // แตรสั้นตอนเลเวลอัพ/โบนัส
      : kind === 'lootbox' ? [392, 523, 659, 784, 988, 1175]  // รัวขึ้นตอนเปิดกล่องสุ่ม
      : [784, 988];
    seq.forEach((f, i) => {
...
function AppProvider({ children, variant, settings }) {
  ...
  // Fun layer: daily streak, weekly loot box, sticker album, mascot outfit.
  // streak.lastDay is a local YYYY-MM-DD; lootbox counts approved missions per week.
  const [streak, setStreak] = useState({ count: 0, best: 0, lastDay: '' });
  const [lootbox, setLootbox] = useState({ week: '', done: 0, lastOpen: '' });
  const [stickers, setStickers] = useState([]);   // sticker IDs, duplicates allowed (collect count)
  ...
  // นับวันทำภารกิจต่อเนื่อง — bump วันละครั้งเมื่อเฟรยาส่งงานเอง (สร้างนิสัยกลับมาเปิดแอป)
  // Milestone days pay a star bonus; the toast is delayed so it doesn't get
  // overwritten by the submit-toast that fires right after.
  const bumpStreak = useCallback(() => {
    const today = dayKey(new Date());
    if (streak.lastDay === today) return;
    const yest = dayKey(new Date(Date.now() - 86400000));
    const count = streak.lastDay === yest ? (streak.count || 0) + 1 : 1;
    setStreak({ count, best: Math.max(count, streak.best || 0), lastDay: today });
    const bonus = STREAK_BONUS[count];
    if (bonus) {
      setStars(s => s + bonus);
      setWallet(w => [{ id: uid('wk'), label: `โบนัสทำต่อเนื่อง ${count} วัน · Streak bonus`, amount: bonus, ts: Date.now() }, ...w]);
      fireConfetti();
      setTimeout(() => {
        showToast(`สุดยอด! ทำต่อเนื่อง ${count} วัน +${bonus} ⭐`, '🔥');
        beepRef.current && beepRef.current('levelup');
      }, 2400);
    } else if (count > 1) {
      setTimeout(() => showToast(`ทำต่อเนื่อง ${count} วันแล้ว!`, '🔥'), 2400);
    }
  }, [streak, fireConfetti, showToast]);

  // นับภารกิจที่อนุมัติแล้วประจำสัปดาห์ (สะสมปลดล็อกกล่องสุ่มรางวัล)
  const bumpWeekly = useCallback(() => {
    const wk = weekKey();
    setLootbox(lb => lb.week === wk ? { ...lb, done: (lb.done || 0) + 1 } : { ...lb, week: wk, done: 1 });
  }, []);

  // เปิดกล่องสุ่มประจำสัปดาห์ — caller (fw-rewards) สุ่มสติกเกอร์เองแล้วส่ง id มา
  const claimLootbox = useCallback((stickerId) => {
    const wk = weekKey();
    if (lootbox.week !== wk || (lootbox.done || 0) < LOOT_GOAL || lootbox.lastOpen === wk) return false;
    setStickers(s => [...s, stickerId]);
    setLootbox(lb => ({ ...lb, lastOpen: wk }));
    beep('lootbox'); fireConfetti();
    return true;
  }, [lootbox, beep, fireConfetti]);

  // ---- ตู้เสื้อผ้ามาสคอต (ของแลกที่ไม่มีต้นทุนจริง) ----
  const buyMascotItem = useCallback((item) => {
    if (mascotFit.owned.includes(item.id)) { showToast('มีชุดนี้แล้ว · Already owned', item.emoji); return; }
    if (stars < item.cost) { showToast('ดาวไม่พอ · Not enough stars', '😅'); beep('pop'); return; }
    setStars(s => s - item.cost);
    setWallet(w => [{ id: uid('wf'), label: 'ชุดมาสคอต · ' + item.en, amount: -item.cost, ts: Date.now() }, ...w]);
    setMascotFit(f => ({ ...f, owned: [...f.owned, item.id], worn: { ...f.worn, [item.slot]: item.id } }));
    beep('reward'); fireConfetti();
    showToast(`ได้ ${item.th} มาแต่งตัวแล้ว!`, item.emoji);
  }, [mascotFit.owned, stars, beep, fireConfetti, showToast]);

  const wearMascotItem = useCallback((item) => {
    setMascotFit(f => {
      const worn = { ...f.worn };
      if (worn[item.slot] === item.id) delete worn[item.slot];
  ...
  }, [missions, beep, fireConfetti, showToast, profile, fbFamily, bumpStreak]);
  ...
  }, [missions, beep, fireConfetti, showToast, bumpWeekly]);
  ...
  }, [beep, showToast, bumpStreak]);

  // Parent approves a submission -> credit stars, bump progress, add to portfolio, mark mission done
  // praise (optional): { text, audioUrl } — คำชมจากคุณแม่ ติดไปกับผลงานในโพลารอยด์และ SAR
  const approveSubmission = useCallback((id, praise) => {
    ...
    setPortfolio(pf => {
      ...
      return [{ id: uid('p'), en: sub.en, th: sub.th, group: sub.group,
        date: thaiDate(now).short, year: now.getFullYear() + 543, ts: now.getTime(), stars: sub.stars, badge: GROUP[sub.group].emoji, inds: sub.inds || [], desc: sub.desc,
        thumb: sub.thumb || null, media: sub.media || 'photo', evidence: sub.evidence || [],
        praise: (praise && praise.text) || '', praiseAudio: (praise && praise.audioUrl) || null }, ...pf];
    });
    ...
    setReviewed(r => [{ id: uid('rv'), th: sub.th, en: sub.en, group: sub.group, stars: sub.stars,
      outcome: 'approved', thumb: sub.thumb || null, when: 'เมื่อสักครู่', ts: Date.now() }, ...r].slice(0, 40));
    setSubmissions(q => q.filter(s => s.id !== id));
  }, [submissions, beep, fireConfetti, showToast, bumpWeekly]);
  ...
  const rejectSubmission = useCallback((id) => {
    ...
    setReviewed(r => [{ id: uid('rv'), th: sub.th, en: sub.en, group: sub.group, stars: sub.stars,
      outcome: 'returned', thumb: sub.thumb || null, when: 'เมื่อสักครู่', ts: Date.now() }, ...r].slice(0, 40));
    setSubmissions(q => q.filter(s => s.id !== id));
    beep('tab');
  ...
  const applySnapshot = useCallback((data) => {
    ...
    if (s.mascotFit) setMascotFit(s.mascotFit);
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        try {
          ...
          if (d.mascotFit) setMascotFit(d.mascotFit);
          beep('reward'); fireConfetti();
  ...
  const val = {
    ...
    streak, lootbox, claimLootbox, stickers,
    mascotFit, buyMascotItem, wearMascotItem,
